 ...
 * a miss reaches the Mistakes list and the page fits a phone.
 *
...
const UNITS = ['Foundations of Algebra', 'Solving Equations', 'Inequalities', 'Functions', 'Linear Equations & Graphs', 'Systems of Equations', 'Exponents & Exponential Functions', 'Polynomials', 'Factoring', 'Quadratic Equations', 'Radicals & the Pythagorean Theorem', 'Statistics'];
...
export default async function algebraChecks({ page, check, fail, STUDY }) {
  ...
  await check('algebra lab', async () => {
    ...
    try {
      ...
      // A hint shows the first step, and a hinted right answer earns nothing.
      ...
      if (!hint || !hint.includes(before.first)) fail(`the hint did not show the first step (${JSON.stringify(hint.slice(0, 80))})`);
      ...
      const gameQs = [];
      for (let i = 0; i < 6; i++) {
        const t = ((await p.locator('.q-card .q-prompt').first().textContent().catch(() => '')) || '').trim();
        gameQs.push(t);
        const opts = p.locator('.q-card .opt:not([disabled])');
        if (!(await opts.count())) break;
        await opts.first().click();
        await p.waitForTimeout(150);
        const next = p.locator('.q-card ~ .row .btn.primary, .panel .row .btn.primary').first();
        if (await next.count()) await next.click().catch(() => {});
        await p.waitForTimeout(500);
      }
      if (!gameQs.some((t) => /^(Simplify|Multiply|Expand):/.test(t))) fail(`no Algebra Lab problems in a unit 8 race: ${JSON.stringify(gameQs)}`);

      // Every unit, the combined set, and a phone.
      for (const id of ['alg1-u9', 'alg1-u5', 'alg1-all']) {
        await go(`/set/${id}/algebra`);
        if (!(await p.locator('.q-card').count())) fail(`no problem rendered on ${id}`);
      }
      await p.setViewportSize({ width: 390, height: 844 });
      for (const id of ['alg1-u2', 'alg1-u3', 'alg1-u11']) {
        await go(`/set/${id}/algebra`);
