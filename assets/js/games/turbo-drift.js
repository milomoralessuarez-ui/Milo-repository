...
(function () {
  ...
  var TRACKS = TRACK_ROWS.map(function (r, i) {
    return {
      ...
      tunnels: r[15], narrow: r[16], walls: !!r[17]
    };
  });
  ...
  function mount(host) {
    ...
    var menuEl = null, hudEl = null, nitroEl = null, msgEl = null;
    ...
    function hideHud() { if (hudEl) { hudEl.remove(); hudEl = null; } }
    ...
    function frame(g, dt) {
      ...
      if (g.state === 'play' && d.built) {
        if (d.countdown > 0) {
          ...
          if (after !== before) Milo.sound.blip();
        ...
        if (msgEl) {
          var text = d.countdown > 0
            ? (Math.ceil(d.countdown) > 0 ? String(Math.ceil(d.countdown)) : 'GO!')
