...
(function () {
  'use strict';
  var Milo = window.Milo, R = Milo.racing, M = Milo.mat4, U = Milo.util;
  ...
  var VS = [
    ...
    '  vec3 base = mix(aCol, uTint, aTint);',
    '  if (uUnlit > 0.5) {',
    '    vCol = base;',
    '  } else {',
    // Rotation lives in the model matrix's upper 3x3; every lit draw uses a
    // uniform scale, so the inverse-transpose would be the same direction.
    '    mat3 nm = mat3(uModel[0].xyz, uModel[1].xyz, uModel[2].xyz);',
    '    vec3 N = normalize(nm * aNrm);',
    '    float diff = max(dot(N, uSun), 0.0);',
    // Hemisphere ambient: sky light from above, warmer bounce from the ground.
    '    float amb = 0.30 + 0.22 * (N.y * 0.5 + 0.5);',
    '    vec3 lit = base * (amb + diff * 0.86);',
    '    vec3 V = normalize(uEye - world.xyz);',
    '    vec3 H = normalize(uSun + V);',
    '    lit += uSunCol * pow(max(dot(N, H), 0.0), 42.0) * uSpec * diff;',
    '    vCol = lit;',
    '  }',
  ...
  /**
   * The car body, minus wheels. Shaped like a single-seater: pointed nose,
   * front and rear wings, sidepods and a roll hoop. Wheels are a separate
   * mesh so they can steer and spin instead of being welded to the shell.
   */
  function buildCarMesh() {
    var m = new Milo.Mesh();
    var TYRE = [.07, .07, .08], CARBON = [.13, .13, .16];

    // Monocoque: a tapering tub from the cockpit forward to the nose.
    m.box(0, .40, -.20, .40, .21, 1.05, GREY, 1);
    m.box(0, .36, .95, .26, .15, .55, GREY, 1, .96);
    m.box(0, .32, 1.52, .15, .11, .35, GREY, 1, .92);

    // Front wing: a wide plane on two short pylons, with endplates.
    m.box(0, .14, 1.92, .78, .035, .30, GREY, 1, 1.04);
    m.box(0, .22, 1.86, .06, .09, .12, CARBON, 0);
    m.box(-.76, .22, 1.92, .035, .15, .30, GREY, 1, .88);
    m.box(.76, .22, 1.92, .035, .15, .30, GREY, 1, .88);

    // Sidepods with their cooling outlets, tapering into the engine cover.
    m.box(-.60, .36, -.30, .24, .19, .72, GREY, 1, .94);
    m.box(.60, .36, -.30, .24, .19, .72, GREY, 1, .94);
    m.box(-.50, .34, -1.02, .13, .14, .34, CARBON, 0);
    m.box(.50, .34, -1.02, .13, .14, .34, CARBON, 0);

    // Cockpit opening, roll hoop and the airbox feeding the engine.
    m.box(0, .60, -.34, .27, .10, .40, GLASS, 0);
    m.box(0, .80, -.62, .17, .22, .14, GREY, 1, .90);
    m.box(0, .66, -.95, .20, .16, .40, GREY, 1, .88);

    // Engine cover falling away to the rear axle.
    m.box(0, .48, -1.28, .22, .16, .36, GREY, 1, .92);

    // Rear wing on twin pylons, with endplates and a lower beam wing.
    m.box(0, 1.00, -1.86, .62, .045, .26, GREY, 1, 1.02);
    m.box(0, .86, -1.84, .05, .18, .10, CARBON, 0);
    m.box(-.60, .86, -1.86, .035, .21, .26, GREY, 1, .86);
    m.box(.60, .86, -1.86, .035, .21, .26, GREY, 1, .86);
    m.box(0, .40, -1.92, .40, .04, .16, CARBON, 0);

    // Diffuser and floor.
    m.box(0, .16, -.40, .56, .035, 1.30, CARBON, 0);
    m.box(0, .22, -1.60, .44, .10, .22, CARBON, 0);
    return m;
  }

  /** Lights, drawn unlit so they read as emitting rather than reflecting. */
  function buildLampMesh(col) {
    var m = new Milo.Mesh();
    m.box(0, 0, 0, 1, 1, 1, col, 0);
    return m;
  }

  /** One wheel: a tyre barrel with a paler rim disc inside it. */
  function buildWheelMesh() {
    var m = new Milo.Mesh();
    m.cylX(0, 0, 0, .19, .36, 12, [.08, .08, .09], 0);
    m.cylX(0, 0, 0, .20, .17, 10, [.55, .57, .62], 0);
    // A spoke pair so the wheel visibly rotates rather than just sitting there.
    m.box(0, 0, 0, .21, .30, .045, [.62, .64, .70], 0);
    m.box(0, 0, 0, .21, .045, .30, [.62, .64, .70], 0);
    return m;
  }

  /** A flat ground quad, used for contact shadows. */
  function buildPuffMesh() {
    var m = new Milo.Mesh();
    m.quad([-1, 0, -1], [-1, 0, 1], [1, 0, 1], [1, 0, -1], GREY, 1, 1, [0, 1, 0]);
    return m;
  }

  /**
   * An upright quad facing +z, yawed toward the camera at draw time. Smoke
   * lying flat on the tarmac reads as a grey slab painted on the road; a
   * billboard reads as smoke.
   */
  function buildSmokeMesh() {
    var m = new Milo.Mesh();
    m.quad([-1, -1, 0], [1, -1, 0], [1, 1, 0], [-1, 1, 0], GREY, 1, 1, [0, 0, 1]);
    return m;
  }

  /**
   * Sky dome. A vertical gradient from the fog colour at the horizon up to the
   * sky colour overhead, with the sun brightening the patch it sits in — a
   * flat clear colour reads as a void, and the horizon never matches the fog.
   */
  function buildSkyMesh(theme, sun) {
    var m = new Milo.Mesh();
    var RINGS = 10, SEGS = 22, R = 3000;
    function at(ring, seg) {
      var el = (ring / RINGS) * (Math.PI / 2);
      var az = (seg / SEGS) * Math.PI * 2;
      var ce = Math.cos(el);
      return [Math.cos(az) * ce * R, Math.sin(el) * R, Math.sin(az) * ce * R];
    }
    function tone(p) {
      var el = Math.max(0, p[1] / R);
      var k = Math.pow(el, .55);
      var c = [
        theme.fog[0] + (theme.sky[0] - theme.fog[0]) * k,
        theme.fog[1] + (theme.sky[1] - theme.fog[1]) * k,
        theme.fog[2] + (theme.sky[2] - theme.fog[2]) * k
      ];
      var l = Math.hypot(p[0], p[1], p[2]) || 1;
      var dot = (p[0] * sun[0] + p[1] * sun[1] + p[2] * sun[2]) / l;
      var glow = Math.pow(Math.max(0, dot), 26) * .8 + Math.pow(Math.max(0, dot), 4) * .16;
      return [Math.min(1, c[0] + glow), Math.min(1, c[1] + glow * .93), Math.min(1, c[2] + glow * .78)];
    }
    for (var r = 0; r < RINGS; r++) {
      for (var sgi = 0; sgi < SEGS; sgi++) {
        var a = at(r, sgi), b = at(r, sgi + 1), c = at(r + 1, sgi + 1), dd = at(r + 1, sgi);
        // Wound to face inward, since the camera sits inside the dome.
        m.tri(a, c, b, tone(a), 1, 0, [0, -1, 0]);
        m.tri(a, dd, c, tone(a), 1, 0, [0, -1, 0]);
      }
    }
    // A skirt below the horizon so a downhill camera never sees past the dome.
    for (var sg2 = 0; sg2 < SEGS; sg2++) {
      var p0 = at(0, sg2), p1 = at(0, sg2 + 1);
      var q0 = [p0[0], -R * .4, p0[2]], q1 = [p1[0], -R * .4, p1[2]];
      m.tri(p0, q1, p1, theme.fog, 1, 0, [0, 1, 0]);
      m.tri(p0, q0, q1, theme.fog, 1, 0, [0, 1, 0]);
    }
    return m;
  }
  ...
  /**
   * Two gravities, deliberately.
   *
   * G is real gravity and is what the tyre model weighs the car with: grip is
   * mu * load, so using anything else makes every cornering speed nonsense.
   * GRAV_AIR is the exaggerated pull used only for airborne arcs, because a
   * jump that hangs for a realistic two seconds feels broken in an arcade
   * racer. Keeping them separate lets the handling be honest while the jumps
   * stay snappy.
   */
  var G = 9.81;
  var GRAV_AIR = 34;

  // Chassis geometry, in metres. A front-heavy CG would understeer forever.
  var WHEELBASE = 3.05;
  var CG_FRONT = 1.62;                     // CG to front axle
  var CG_REAR = WHEELBASE - CG_FRONT;      // CG to rear axle
  var CG_HEIGHT = .32;
  var WHEEL_R = .36;

  // Aero. Both scale with v^2: drag sets top speed, downforce buys cornering
  // grip that a slow car simply does not have.
  var DRAG_K = .62;
  var LIFT_K = 1.05;
  var ROLL_RESIST = 11;

  var GEARS = [2.92, 2.10, 1.62, 1.30, 1.09, .94, .82];
  var RPM_IDLE = 1900, RPM_MAX = 13500;

  /** Engine revs for a given road speed in a given gear. */
  function revsAt(P, speed, gear) {
    return Math.abs(speed) / WHEEL_R * (60 / (Math.PI * 2)) * GEARS[gear] * P.finalDrive;
  }

  /** Normalised torque curve: soft low down, peak at ~72% of the rev range. */
  function torqueShape(rpm) {
    var x = U.clamp((rpm - RPM_IDLE) / (RPM_MAX - RPM_IDLE), 0, 1);
    return .52 + .48 * Math.sin(Math.min(1, x / .72) * Math.PI * .5) - .28 * Math.max(0, x - .82) / .18;
  }

  /**
   * Simplified Pacejka: force rises steeply with slip angle, peaks near the
   * limit, then falls away. That fall-off is what makes a slide something you
   * have to catch rather than a state the car sits in happily.
   */
  function tyreForce(slip, load, mu, stiff) {
    var B = stiff || 8.4, C = 1.5, D = mu * load;
  ...
  function physicsFor(stats) {
    var top = 27 + stats.top * 43;
    return {
      top: top,
      // Peak power is derived from the target top speed so that drag alone
      // sets it: P = k * v^3 at terminal velocity.
      power: DRAG_K * top * top * top,
      mass: 660 + stats.mass * 560,
      mu: 1.12 + stats.grip * .62,
      brakeTorque: 4200 + stats.brake * 3800,
      // Steering lock, and how much of it survives at speed.
      lock: .46 + stats.grip * .10,
      // Geared for its own top speed, the way a real car is: top gear must
      // reach the limiter right as drag stops it accelerating, so every car
      // uses its whole rev range instead of idling at maximum speed.
      finalDrive: RPM_MAX * .96 /
        (top / WHEEL_R * (60 / (Math.PI * 2)) * GEARS[GEARS.length - 1]),
  ...
  /** Peak lateral acceleration available right now, aero included. */
  function gripLimit(P, speed) {
    var load = P.mass * G + LIFT_K * speed * speed;
    return P.mu * load / P.mass;
  }
  ...
  function mount(host) {
    ...
    function initGpu(g) {
      ...
      gpu = {
        ...
        loc: {
          ...
          sunCol: gl.getUniformLocation(prog, 'uSunCol'),
        ...
        sky: gl.createBuffer(), skyN: 0,
        car: gl.createBuffer(), carN: 0,
        wheel: gl.createBuffer(), wheelN: 0,
        lamp: gl.createBuffer(), lampN: 0,
        puff: gl.createBuffer(), puffN: 0,
        smoke: gl.createBuffer(), smokeN: 0
      };
      upload(gpu.car, carMesh); gpu.carN = carMesh.count();
      upload(gpu.wheel, wheelMesh); gpu.wheelN = wheelMesh.count();
      upload(gpu.lamp, lampMesh); gpu.lampN = lampMesh.count();
      upload(gpu.puff, puffMesh); gpu.puffN = puffMesh.count();
      upload(gpu.smoke, smokeMesh); gpu.smokeN = smokeMesh.count();
    ...
    function uploadTrack(g) {
      ...
      var skm = buildSkyMesh(cur.theme, cur.theme.sun);
      upload(gpu.sky, skm);
      gpu.skyN = skm.count();
      cur.dirty = false;
    }
    ...
    function bindMesh(buf) {
      var gl = gpu.gl, L = gpu.loc, stride = Milo.Mesh.STRIDE * 4;
      gl.bindBuffer(gl.ARRAY_BUFFER, buf);
      ...
      gl.vertexAttribPointer(L.nrm, 3, gl.FLOAT, false, stride, 24);
      gl.enableVertexAttribArray(L.tint);
      gl.vertexAttribPointer(L.tint, 1, gl.FLOAT, false, stride, 36);
    }
    ...
    function showHud(g) {
      ...
      var dash = el('div', 'td-dash');
      tachEl = el('div', 'td-tach', '<b></b>');
      gearEl = el('div', 'td-gear', '1');
      dash.appendChild(tachEl);
      dash.appendChild(gearEl);
      hudEl.appendChild(lightsEl);
      hudEl.appendChild(msgEl);
      hudEl.appendChild(dash);
    ...
    function hideHud() {
      ...
      lightsEl = null; tachEl = null; gearEl = null;
    }
    ...
    function aiDrive(car, built) {
      ...
      // Slow to whatever the tightest corner in view actually allows, read
      // off the same tyre-and-aero budget the physics uses. Rivals lift a
      // little early: a car that understeers into the outside barrier is
      // slower than one that lifts.
      var limit = car.phys.top;
      for (var k = 3; k < look + 34; k += 3) {
        var sm = s[(car.si + k) % n];
        var c = Math.abs(sm.curvS);
        if (c <= 1e-5) continue;
        // Corner speed where lateral demand meets available grip, solved at
        // that speed rather than this one, since downforce grows with it.
        var ay = gripLimit(car.phys, Math.min(speed + k * .4, car.phys.top)) * .80 * car.skill;
        ay += G * Math.abs(Math.sin(sm.bank || 0)) * .5;
        limit = Math.min(limit, Math.sqrt(ay / c) + k * .30);
      }
    ...
    function step(g, dt) {
      ...
      d.cars.forEach(function (car) {
        ...
        // --- steering input ------------------------------------------------
        // Keyboard steering is on or off, so the wheel is ramped toward the
        // input rather than snapped to it; a real driver cannot reach full
        // lock instantly either. Lock also falls away with speed, which is
        // what stops a flick of the key spinning the car on a straight.
        var speed = Math.hypot(car.vx, car.vz);
        var lockNow = P.lock * (1 - .70 * U.clamp(speed / P.top, 0, 1));
        var rate = steer === 0 ? 5.2 : 3.4;
        car.steerPos = car.steerPos == null ? 0 : car.steerPos;
        car.steerPos += U.clamp(steer - car.steerPos, -rate * dt, rate * dt);
        var delta = car.steerPos * lockNow;
        ...
        if (car.ground) {
          var m = P.mass;
          var boostK = boosting ? 1.35 : 1;

          // --- gearbox: pick a ratio, read the revs off the driveshaft -----
          car.gear = car.gear == null ? 0 : car.gear;
          var rpm = U.clamp(revsAt(P, vf, car.gear), RPM_IDLE, RPM_MAX);
          car.shiftT = Math.max(0, (car.shiftT || 0) - dt);
          if (!car.shiftT) {
            if (rpm > RPM_MAX * .96 && car.gear < GEARS.length - 1) { car.gear++; car.shiftT = .12; }
            else if (rpm < RPM_MAX * .42 && car.gear > 0) { car.gear--; car.shiftT = .10; }
          }
          car.rpm = rpm;

          // --- longitudinal demand -----------------------------------------
          // Power-limited above a crawl, torque-limited below it.
          var driveF = 0;
          if (thr > 0 && !car.shiftT) {
            driveF = P.power * boostK * torqueShape(rpm) / Math.max(Math.abs(vf), 7);
          }
          var brakeF = 0;
          if (brk > 0) {
            if (vf > .6) brakeF = P.brakeTorque / WHEEL_R;
            else driveF = -P.mass * 2.6;              // crawl backwards
          }

          // --- vertical load, with transfer and downforce ------------------
          var downforce = LIFT_K * speed * speed;
          var axPrev = car.ax || 0;
          var staticF = m * G * (CG_REAR / WHEELBASE), staticR = m * G * (CG_FRONT / WHEELBASE);
          // Accelerating pitches load onto the rear axle, braking onto the front.
          var transfer = m * axPrev * CG_HEIGHT / WHEELBASE;
          var loadF = Math.max(120, staticF - transfer + downforce * .44);
          var loadR = Math.max(120, staticR + transfer + downforce * .56);

          // --- slip angles --------------------------------------------------
          // At walking pace the slip-angle formulation is numerically useless,
          // so below a few m/s the model blends into plain kinematic steering.
          var vsafe = Math.max(Math.abs(vf), 2.2);
          var slipF = Math.atan2(vr + car.yawRate * CG_FRONT, vsafe) - delta * (vf < 0 ? -1 : 1);
          var slipR = Math.atan2(vr - car.yawRate * CG_REAR, vsafe);

          var edgy = Math.abs(car.off) > car.roadW * .84;
          var muNow = P.mu * (edgy ? .74 : 1) * (sm.kind === 2 ? 1.04 : 1);

          // The rear axle is deliberately the stiffer end. A car that lets go
          // at the back first is a car nobody can catch on a keyboard.
          var Fyf = tyreForce(slipF, loadF, muNow, 8.2);
          var Fyr = tyreForce(slipR, loadR, muNow, 10.4);

          var capR = muNow * loadR;
          var capF = muNow * loadF;

          // Traction and brake control. These are real systems on a real race
          // car, and here they are also what makes the model driveable: a
          // keyboard throttle is fully open or fully shut, so without them the
          // car would sit permanently at the longitudinal limit with nothing
          // left for cornering, and every corner would end in a spin.
          driveF /= 1 + Math.max(0, Math.abs(slipR) - .09) * 7;
          driveF = U.clamp(driveF, -capR * .92, capR * .92);
          brakeF = Math.min(brakeF, (capF + capR) * .88);

          // --- friction ellipse: a tyre spending grip forwards has less
          //     left for cornering, which is why you cannot brake and turn.
          var rearLong = driveF - brakeF * .38;
          var frontLong = -brakeF * .62;
          // Whatever is still beyond the contact patch is simply not delivered.
          car.slipping = Math.abs(rearLong) > capR * .9;
          rearLong = U.clamp(rearLong, -capR, capR);
          frontLong = U.clamp(frontLong, -capF, capF);
          var availR = Math.sqrt(Math.max(0, capR * capR - rearLong * rearLong));
          var availF = Math.sqrt(Math.max(0, capF * capF - frontLong * frontLong));
          Fyr = U.clamp(Fyr, -availR, availR);
          Fyf = U.clamp(Fyf, -availF, availF);

          // --- resistance ---------------------------------------------------
          var drag = DRAG_K * vf * Math.abs(vf);
          var roll = ROLL_RESIST * vf;
          var Fx = rearLong + frontLong - drag - roll;

          // --- accelerations -------------------------------------------------
          var ax = Fx / m;
          var ay = (Fyf * Math.cos(delta) + Fyr) / m;
          // Banking tilts the contact patch, so part of the car's weight helps
          // it turn — which is the whole point of a banked corner.
          ay += G * Math.sin(car.roadBank || 0);
          car.ax = ax;

          vf += (ax + vr * car.yawRate) * dt;
          vr += (ay - vf * car.yawRate) * dt;

          var Iz = m * 1.62;
          var yawAcc = (CG_FRONT * Fyf * Math.cos(delta) - CG_REAR * Fyr) / Iz;
          car.yawRate += yawAcc * dt;

          // Below a crawl the bicycle model has nothing to work with; steer
          // kinematically so parking and slow hairpins still respond.
          if (Math.abs(vf) < 4.5) {
            var kin = vf * Math.tan(delta) / WHEELBASE;
            var blend = 1 - Math.abs(vf) / 4.5;
            car.yawRate += (kin - car.yawRate) * blend * Math.min(1, 9 * dt);
          }
          car.yawRate *= Math.exp(-.9 * dt);       // yaw damping from the tyres

          // Stability control: bleed off yaw the tyres could never sustain at
          // this speed. Oversteer still happens and still has to be caught —
          // this only stops an unrecoverable spin from a single key press.
          var yawCap = gripLimit(P, Math.abs(vf)) / Math.max(Math.abs(vf), 6) * 1.35;
          if (Math.abs(car.yawRate) > yawCap) {
            car.yawRate += (Math.sign(car.yawRate) * yawCap - car.yawRate) * Math.min(1, 5 * dt);
          }
          car.yaw += car.yawRate * dt;

          car.drift = Math.abs(vr);
          car.slipAngle = Math.abs(slipR);
          if ((car.drift > 4.2 || (car.slipping && Math.abs(vf) < 26)) &&
            speed > 11 && Math.random() < .4) puff(d, car, false);
        } else {
          // Airborne: no contact patch, so only drag acts, and the driver has
          // just enough authority to line the car up for the landing.
          car.yaw += steer * P.air * .55 * dt;
          car.yawRate *= Math.exp(-2.0 * dt);
          vf -= DRAG_K * vf * Math.abs(vf) / P.mass * dt;
          vr -= vr * .35 * dt;
          car.drift = 0;
          car.slipping = false;
        }
        ...
        // Wheels turn at road speed; a locked or spinning tyre does not.
        var spin = car.slipping && thr > 0 ? vf * 1.9 : (brk > 0 && car.ground && Math.abs(vf) > 3 ? vf * .25 : vf);
        car.wheelAng = (car.wheelAng || 0) + spin / WHEEL_R * dt;
        car.braking = brk > 0;
      ...
      // Contact between cars. The overlap is pushed apart by mass, and the
      // velocity change is an impulse along the contact normal, applied only
      // while the two are closing. Scaling each car's whole velocity on every
      // frame of contact instead — which is what this used to do — brings a
      // queue of nineteen cars to a standstill within a second of the start.
      var CONTACT = 2.7, RESTITUTION = .18;
      for (var a = 0; a < d.cars.length; a++) {
        for (var b = a + 1; b < d.cars.length; b++) {
          ...
          if (dist > CONTACT || dist < .001) continue;
          if (Math.abs(c1.y - c2.y) > 2.5) continue;
          var nx = dx / dist, nz = dz / dist;
          var inv1 = 1 / c1.phys.mass, inv2 = 1 / c2.phys.mass, invSum = inv1 + inv2;

          var overlap = CONTACT - dist;
          c1.x -= nx * overlap * (inv1 / invSum); c1.z -= nz * overlap * (inv1 / invSum);
          c2.x += nx * overlap * (inv2 / invSum); c2.z += nz * overlap * (inv2 / invSum);

          var vn = (c2.vx - c1.vx) * nx + (c2.vz - c1.vz) * nz;
          if (vn >= 0) continue;                       // already separating
          var j = -(1 + RESTITUTION) * vn / invSum;
          c1.vx -= nx * j * inv1; c1.vz -= nz * j * inv1;
          c2.vx += nx * j * inv2; c2.vz += nz * j * inv2;
          if ((c1.player || c2.player) && -vn > 4) Milo.sound.hit();
        }
      }
    }
    ...
    function render(g, wall) {
      ...
      if (me && g.state !== 'start') {
        ...
        // Close and low: the old camera sat so far back and so high that the
        // car read as a dot and the road lost all sense of speed.
        var back = 8.0 + k * 2.6;
        var want = [me.x - fwdX * back, me.y + 2.55 + k * .55, me.z - fwdZ * back];
        ...
        eye = d.cam;
        look = [me.x + fwdX * (9 + k * 5), me.y + 1.05, me.z + fwdZ * (9 + k * 5)];
        // The horizon leans out of a slide, which is most of what sells the
        // difference between gripping and sliding from behind the car.
        var vrNow = me.vx * Math.cos(me.yaw) - me.vz * Math.sin(me.yaw);
        var wantRoll = U.clamp(-vrNow * .006, -.09, .09);
        d.camRoll = (d.camRoll || 0) + (wantRoll - (d.camRoll || 0)) * Math.min(1, 4 * wall);
        roll = d.camRoll;
        g.data.camK = k;
      } else {
      ...
      var proj = M.perspective(fov, aspect, .5, 4200);
      var view = M.lookAt(eye[0], eye[1], eye[2], look[0], look[1], look[2],
      ...
      gl.uniform3f(L.sun, theme.sun[0], theme.sun[1], theme.sun[2]);
      gl.uniform3f(L.sunCol, theme.sunCol[0], theme.sunCol[1], theme.sunCol[2]);
      gl.uniform2f(L.fog, 170, 900);
      gl.uniform3f(L.fogColor, theme.fog[0], theme.fog[1], theme.fog[2]);
      gl.uniform1f(L.alpha, 1);
      gl.uniform3f(L.uTint, 1, 1, 1);

      // --- sky: drawn first, unlit, and carried along with the camera -------
      if (gpu.skyN) {
        gl.depthMask(false);
        gl.disable(gl.CULL_FACE);
        gl.uniform1f(L.unlit, 1);
        gl.uniform1f(L.spec, 0);
        gl.uniformMatrix4fv(L.model, false, M.model(eye[0], eye[1], eye[2], 0, 0, 0, 1, 1, 1));
        bindMesh(gpu.sky);
        gl.drawArrays(gl.TRIANGLES, 0, gpu.skyN);
        gl.depthMask(true);
        gl.enable(gl.CULL_FACE);
      }
      ...
      if (d && d.cars) {
        // --- contact shadows ------------------------------------------------
        // Without these every car looks pasted on top of the track rather than
        // standing on it. They sit on the road, not on the car, so an airborne
        // car's shadow stays down on the tarmac where it belongs.
        gl.enable(gl.BLEND);
        gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
        gl.depthMask(false);
        gl.uniform1f(L.unlit, 1);
        gl.uniform3f(L.uTint, .04, .05, .09);
        bindMesh(gpu.puff);
        d.cars.forEach(function (car) {
          if (car.respawnT > 0) return;
          var lift = Math.max(0, car.y - (car.roadY || car.y));
          var sc = 1 + lift * .10;
          gl.uniform1f(L.alpha, U.clamp(.46 - lift * .05, .06, .46));
          gl.uniformMatrix4fv(L.model, false,
            M.model(car.x, (car.roadY || car.y) + .05, car.z, car.yaw, 0, car.roadBank || 0,
              1.15 * sc, 1, 2.3 * sc));
        ...
        // --- bodies ------------------------------------------------------
        gl.uniform1f(L.unlit, 0);
        gl.uniform1f(L.spec, .55);           // clear-coat highlight on paint
        bindMesh(gpu.car);
        var models = [];
        d.cars.forEach(function (car) {
          if (car.respawnT > 0 && Math.floor(car.respawnT * 12) % 2) { models.push(null); return; }
        ...
        // --- wheels, close up only ------------------------------------------
        // Four extra draws a car is worth it near the camera and invisible far
        // away, so the field stays cheap to render.
        gl.uniform1f(L.spec, .18);
        gl.uniform3f(L.uTint, 1, 1, 1);
        bindMesh(gpu.wheel);
        var WHEELS = [[-.86, 1.30, 1], [.86, 1.30, 1], [-.92, -1.34, 0], [.92, -1.34, 0]];
        d.cars.forEach(function (car, ci) {
          var model = models[ci];
          if (!model) return;
          if (U.dist(car.x, car.z, eye[0], eye[2]) > 150) return;
          for (var w = 0; w < 4; w++) {
            var P4 = WHEELS[w];
            var steerY = P4[2] ? (car.steerAngle || 0) : 0;
            gl.uniformMatrix4fv(L.model, false,
              M.mul(model, M.model(P4[0], WHEEL_R, P4[1], steerY, car.wheelAng || 0, 0, 1, 1, 1)));
            gl.drawArrays(gl.TRIANGLES, 0, gpu.wheelN);
          }
        });
        ...
        d.cars.forEach(function (car, ci) {
          var model = models[ci];
          if (!model) return;
          if (U.dist(car.x, car.z, eye[0], eye[2]) > 180) return;
          var hot = car.braking ? 1 : .18;
          gl.uniform3f(L.uTint, hot, hot * .18, hot * .14);
          gl.uniformMatrix4fv(L.model, false,
            M.mul(model, M.model(0, .40, -2.00, 0, 0, 0, .16, .07, .05)));
          gl.drawArrays(gl.TRIANGLES, 0, gpu.lampN);
        });
        ...
        if (d.puffs.length) {
          ...
          d.puffs.forEach(function (p) {
            var f = p.life / p.max;
            var sc = (1.5 - f) * (p.hot ? .85 : 1.25);
            var face = Math.atan2(eye[0] - p.x, eye[2] - p.z);
            gl.uniformMatrix4fv(L.model, false,
              M.model(p.x, p.y + .5 + (1 - f) * 1.6, p.z, face, 0, 0, sc, sc, 1));
            if (p.hot) gl.uniform3f(L.uTint, 1, .62, .25);
            else gl.uniform3f(L.uTint, .82, .84, .90);
            gl.uniform1f(L.alpha, f * (p.hot ? .5 : .26));
            gl.drawArrays(gl.TRIANGLES, 0, gpu.smokeN);
          });
          gl.uniform1f(L.alpha, 1);
          gl.depthMask(true);
          gl.enable(gl.CULL_FACE);
    ...
    function frame(g, dt) {
      ...
      if (g.state === 'play' && d.built) {
        ...
        if (tachEl) {
          var rev = U.clamp(((me.rpm || RPM_IDLE) - RPM_IDLE) / (RPM_MAX - RPM_IDLE), 0, 1);
          tachEl.firstChild.style.width = Math.round(rev * 100) + '%';
          var red = rev > .93;
          if (tachEl.classList.contains('redline') !== red) tachEl.classList.toggle('redline', red);
        }
        if (gearEl) {
          var gtxt = 'N';
          if (Math.abs(me.vx) + Math.abs(me.vz) > .5 || me.gear > 0) gtxt = String((me.gear || 0) + 1);
          if (gearEl.textContent !== gtxt) gearEl.textContent = gtxt;
        }
      }
